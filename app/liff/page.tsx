...
export default function LiffBootstrapPage() {
  ...
  const { t } = useLanguage()
  const [status, setStatus] = useState<'initializing' | 'logging_in' | 'error' | 'not_in_line' | 'not_friend'>('initializing')
  ...
  useEffect(() => {
    const initializeLiff = async () => {
      ...
      try {
        ...
        // Send userId to backend (now with friendship confirmed)
        ...
        // Check query param returnTo first, then LIFF context path
        const urlParams = new URLSearchParams(window.location.search)
        const returnTo = urlParams.get('returnTo')

        const context = liff.getContext() as { path?: string } | null
        const rawPath = returnTo || context?.path || ''
        ...
        const validPaths = ['/order/status/', '/order/status', '/order/menu', '/order/cart', '/order/checkout', '/order/payment', '/order/confirmed', '/order/edit/', '/order/closed']
        const isValidPath = validPaths.some(p => {
          // Exact match or startsWith for paths that can have params
          if (p === '/order/status/' || p === '/order/edit/') {
  ...
  const handleAddFriend = () => {
    ...
    try {
      // Try to add friend via LIFF SDK (preferred method)
      if (typeof liffInstance.addFriend === 'function') {
        liffInstance.addFriend()
      } else {
        // Fallback: open OA profile page in external browser
        ...
        if (oaId) {
          ...
          console.error('[LIFF] LINE OA ID not configured')
          alert(t('liffNotConfigured'))
        }
      }
    } catch (err) {
      console.error('[LIFF] Add friend error:', err)
    }
  }
      ...
      <div className="min-h-screen bg-bg flex items-center justify-center px-5">
        <div className="text-center max-w-md">
          ...
          <h1 className="text-2xl font-bold text-text mb-3">
            {t('addFriendRequired') || 'เพิ่มเพื่อนเพื่อดำเนินการต่อ'}
          </h1>
          <p className="text-sm text-muted mb-8 leading-relaxed">
            {t('addFriendDescription') || 'กรุณาเพิ่ม Official Account เป็นเพื่อนเพื่อใช้บริการสั่งอาหาร'}
          </p>
          <button
            onClick={handleAddFriend}
            className="w-full py-4 px-6 bg-primary text-white font-semibold rounded-lg hover:bg-primary/90 active:bg-primary/80 transition-colors"
          >
            {t('addFriend') || 'เพิ่มเพื่อน'}
          </button>
        </div>
      </div>
    )
