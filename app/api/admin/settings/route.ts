...
export async function GET(request: NextRequest) {
  ...
  try {
    const { data, error } = await supabase
      .from('admin_settings')
      .select('id, line_approver_id, line_staff_id, pin_version')
      .limit(1)
      .single()
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { line_approver_id, line_staff_id, new_staff_pin } = body

    // Validate inputs
    if (!line_approver_id || !line_staff_id) {
      return NextResponse.json({ error: 'LINE IDs cannot be empty' }, { status: 400 })
    }
