...
export default function MenuPage() {
  ...
  const itemsByCategory = categories.reduce((acc, category) => {
    acc[category] = (menuData as MenuItem[]).filter(item => item.category === category)
    return acc
  ...
  useEffect(() => {
    ...
    if (savedPosition) {
      ...
      // Use requestAnimationFrame for more reliable timing on iOS Safari
      requestAnimationFrame(() => {
        ...
        // Keep observer disabled briefly after scroll restoration
  ...
  const handleCategoryClick = (category: Category) => {
    ...
    if (element) {
      const headerHeight = 73 // BrandHeader height
      const tabsHeight = 68 // Category tabs height (44px button + 24px padding)
  ...
  // Handle category picker selection
